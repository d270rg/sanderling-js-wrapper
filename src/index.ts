...
async function waitForGameClientProcess(): Promise<ListGameClientProcessesResponse> {
  const p = new Promise<ListGameClientProcessesResponse>((resolve, reject) => {
    ...
    const interval = setIntervalAsync(async () => {
      console.log(`Waiting for game client > ${checkIterations}`);
      checkIterations++;
      ...
      console.log(
        result,
        typeof result,
        result.ListGameClientProcessesResponse
      );
      if (result && result.ListGameClientProcessesResponse !== undefined) {
        ...
        clearIntervalAsync(interval);
        console.log(
          `Game client with process id ${result.ListGameClientProcessesResponse[0].processId} retrieved`
        );
        resolve(result);
      }
    }, 1000);
...
async function waitForUIRootAddress(processId: number): Promise<string> {
  const p = new Promise<string>((resolve, reject) => {
    let checkIterations = 0;
    const interval = setIntervalAsync(async () => {
      console.log(`Waiting for UI root address > ${checkIterations}`);
      checkIterations++;
...
// --------------------- Workload ---------------------------
async function getMemoryReadResult() {
  ...
  const readResult = await readFromWindow(
  ...
  if (!readResult) {
    throw new Error("No read result retrieved from process");
  }
  console.log(readResult);
}

void getMemoryReadResult();
